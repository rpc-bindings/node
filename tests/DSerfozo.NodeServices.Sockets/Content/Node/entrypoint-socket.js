...
var TryRequire_1 = __webpack_require__(12);
...
virtualConnectionServer.createInterface(server).on('connection', function (connection) {
    var rl = readline.createInterface(connection, null).on('line', function (line) {
        try {
            // Get a reference to the function to invoke
            var invocation = JSON.parse(line);
            var invokedModule = TryRequire_1.tryRequire(invocation.moduleName);
            if (!invokedModule) {
                invokedModule = TryRequire_1.tryRequire(path.resolve(process.cwd(), invocation.moduleName));
            }
